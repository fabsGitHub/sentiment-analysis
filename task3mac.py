...
import gc
import os
import time  # Added for tracking runtimes
import pandas as pd
import numpy as np
...
# ... (Keep all your MONTH_MAP, WRITTEN_NUMS, CLEANUP arrays, and text functions exactly as they are) ...
MONTH_MAP = {
...
PRESERVED_WORDS = {'below', 'but', 'down', 'few', 'more', 'no', 'nor', 'not', 'only', 'over', 'should', 'up'}
...
EARLY_CLEANUP = [(re.compile(r"(x[0-9a-fA-F]{4}|[^\x00-\x7F]+)"), " "),(re.compile(r"(\d+)(st|nd|rd|th)", re.I), r"\1"),(re.compile(r"(\d)\s(\d)"), r"\1\2"),(re.compile(r"(\.)\s(\d)"), r"\1\2"),(re.compile(r"(\d)\s(\.)"), r"\1\2"),]
CURRENCIES = r"eur|usd|gbp|jpy|chf|sek|eek"
FINANCIAL_CLEANUP = [(re.compile(r"\bus\s*\$", re.I), "usd"),(re.compile(r"\beuros?(?=\d)", re.I), "eur "),(re.compile(r"\beuros?\b", re.I), "eur"),(re.compile(r"\ber\b", re.I), "eur"),(re.compile(r"x20ac"), "eur"),(re.compile(r"\$"), "usd"),(re.compile(r"\%"), "pct"),(re.compile(r"\b(\d+\.?\d*)\s*us\s*million\b", re.I), r"\1mn"),(re.compile(r"\b(\d+\.?\d*)\s*us\s*m\b", re.I), r"\1mn"),(re.compile(r"(\d+\.?\d*)\s*(percent|per cent)", re.I), r"\1pct"),(re.compile(r"\bsek\b", re.I), "eur"),(re.compile(r"\bmln\b", re.I), "mn"),(re.compile(r"\b(\d+\.?\d*)\s*billion\b", re.I), r"\1bn"),(re.compile(r"\b(\d+\.?\d*)\s*million\b", re.I), r"\1mn"),(re.compile(rf"\b({CURRENCIES})\s*(\d+\.?\d*)\s*\bm\b", re.I), r"\1\2mn"),(re.compile(rf"\b(\d+\.?\d*)\s*\bm\s*({CURRENCIES})\b", re.I), r"\2\1mn"),(re.compile(rf"\b({CURRENCIES})\s*(\d+\.?\d*)\s*m\b", re.I), r"\1\2mn"),(re.compile(r"({CURRENCIES})\s*([-+]?\d+\.?\d*)\s*(m|mn|bn|k|pct|%)", re.I), r"\1\2\3"),(re.compile(r"([-+]?\d+\.?\d*)\s*(m|mn|bn|k|pct|%)", re.I), r"\1\2"),(re.compile(rf"({CURRENCIES})\s*([-+]?\d+\.?\d*)", re.I), r"\1\2"),(re.compile(rf"([-+]?\d+\.?\d*)\s*({CURRENCIES})(?!\d)", re.I), r"\2\1"),(re.compile(rf"\b(\d+\.?\d*)\s*(m|mn|bn|k|pct)\s*({CURRENCIES})\b", re.I), r"\3\1\2"),(re.compile(rf"({CURRENCIES})(\d+)\s*,\s*(\d+)\s*(m|mn|bn|k)", re.I), r"\1\2,\3\4"),(re.compile(r"(\d+)(pct|mn|bn|k|%)\s*-\s*(\d+)\2", re.I), r"\1-\3\2"),(re.compile(r"(\d+),(\d+)"), r"\1.\2"),]
LATE_CLEANUP = [(re.compile(r"(\d{1,2}:\d{2})\s*(am|pm)\b", re.I), r"\1\2"),(re.compile(r"\bsq\s*m\b", re.I), "sqm"),(re.compile(r"(\d+)\s*(sqm|m|km|kg|g)", re.I), r"\1\2"),(re.compile(r"\b([a-zA-Z]+)\s*(\d{1,2})\s*-\s*([a-zA-Z]+)\s*(\d{1,2})\b"), r"\1\2-\3\4"),(re.compile(r"(?<!\d)(\d{4})-(\d{2})(?!\d|:)"), r"\1-20\2"),(re.compile(r"\b(\d{1,2})-(\d{4})\b"), lambda m: f"{m.group(2)}-{m.group(1).zfill(2)}"),(re.compile(r"\s+"), " "), (re.compile(r"\s*'(\w+)"), ""),]
...
def preprocess_full(text):
    ...
    for word, tag in tagged_tokens:
        ...
        if lemma not in CUSTOM_STOPWORDS: lemmatized_tokens.append(lemma)
...
def preprocess_masked(text):
    ...
    tokens = text.split()
    tagged_tokens = pos_tag(tokens)
    mask_placeholders = {'[PHONE]', '[MONEY]', '[PERCENT]', '[DATE]', '[TIME]', '[MEASUREMENT]', '[NUMBER]'}
    processed_tokens = []
    for word, tag in tagged_tokens:
        if word in mask_placeholders: processed_tokens.append(word)
            ...
            word_pos = get_wordnet_pos(tag)
            lemma = lemmatizer.lemmatize(word, pos=word_pos)
            if lemma not in CUSTOM_STOPWORDS: processed_tokens.append(lemma)
...
# ==========================================
# RE-ENGINEERED HIGH-PERFORMANCE PYTORCH BACKEND
# ==========================================
...
if __name__ == "__main__":
    ...
    print("[STATUS] Loading corpus 'Sentences_50Agree.txt'...")
    try:
        df = pd.read_csv("Sentences_50Agree.txt", sep="@", header=None, names=["sentence", "sentiment"])
        print(f"[STATUS] Dataset loaded successfully. Shape: {df.shape}")
    ...
    n_cores = -1
    print(f"[STATUS] Spawning asynchronous task structures across all core clusters...")

    # --- Preprocessing Block with exact timer loops ---
    t0 = time.time()
    df["prep_standard"] = Parallel(n_jobs=n_cores)(delayed(preprocess_standard)(text) for text in df["sentence"])
    print(f"  -> 'prep_standard' routine completed in {time.time() - t0:.2f}s")

    t0 = time.time()
    df["prep_full"] = Parallel(n_jobs=n_cores)(delayed(preprocess_full)(text) for text in df["sentence"])
    print(f"  -> 'prep_full' routine completed in {time.time() - t0:.2f}s")

    t0 = time.time()
    df["prep_masked"] = Parallel(n_jobs=n_cores)(delayed(preprocess_masked)(text) for text in df["sentence"])
    print(f"  -> 'prep_masked' routine completed in {time.time() - t0:.2f}s")

    t0 = time.time()
    df["prep_standard_numbers"] = Parallel(n_jobs=n_cores)(delayed(preprocess_standard_optimized)(text) for text in df["sentence"])
    print(f"  -> 'prep_standard_numbers' routine completed in {time.time() - t0:.2f}s")
    ...
    ngram_ranges = [(1, 4), (1, 2), (1, 1), (1, 3)]
    custom_token_pattern = r'(?u)\[?\b\w[-\w\.]*\b\]?'
    master_results = []

    total_pipeline_timer = time.time()
    ...
    for strategy in strategies:
        ...
        for ngram_range in ngram_ranges:
            ...
            for sampler_name, sampler in samplers.items():
                ...
                adjusted_n_iter = min(64, total_combinations)

                print(f"       -> [SEARCH] Launching RandomizedSearchCV (Evaluating {adjusted_n_iter} variations in parallel)...")
                ...
                print(f"          * Search loop finished in {time.time() - search_start:.2f}s.")
                print(f"          * Top Params: {search_engine.best_params_}")

                best_model = search_engine.best_estimator_
                y_pred = best_model.predict(X_test_vec)
                acc, macro_f1 = evaluate_model(y_test, y_pred, f"{strategy} + {sampler_name}")
                print(f"          * Test Results -> Accuracy: {acc:.4f} | Macro F1: {macro_f1:.4f}")
    ...
    print(f"\n[STATUS] Total structural benchmark completed in {(time.time() - total_pipeline_timer)/60:.2f} minutes.")
    ...
    print("\n[STATUS] Final spreadsheet matrix saved to 'sentiment_analysis_results.csv'. Execution complete.")
